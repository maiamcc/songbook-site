...
export function loadEnums(yamlStr) {
  ...
  for (const [field, body] of Object.entries(raw || {})) {
    ...
    const values = normalizeEnumValues(body.values, field);
    const entry = { desc: body.desc ?? null, values };
    if (body.abbrs != null) {
      if (typeof body.abbrs !== "object" || Array.isArray(body.abbrs)) {
        throw new Error(`enums.yaml: "${field}.abbrs" must be a map`);
      }
      for (const k of Object.keys(body.abbrs)) {
        if (!(k in values)) {
          throw new Error(`enums.yaml: "${field}.abbrs" key "${k}" is not a legal value`);
        }
      }
      entry.abbrs = Object.fromEntries(
        Object.entries(body.abbrs).map(([k, v]) => [k, String(v)])
      );
    }
    out[field] = entry;
  }
...
//     indexable: true,
//   })
export function enumField({ desc, values, abbrs, required, display, indexable, collapsedOn, check, filterable, coerceInt, sortByOrder }) {
...
//     indexable: true,
//   })
export function listEnumField({ desc, values, abbrs, required, display, indexable, collapsedOn, filterable }) {
  const keys = Object.keys(values);
  const valid = new Set(keys);
  const entry = {
    required,
...
export const FIELDS = {
  ...
  // Computed from the markdown body in buildFilterRecord; never set in
  // frontmatter. false = song has no lyrics body at all.
  has_lyrics: {
    required: false,
    type: "boolean",
    check: isBoolean,
  ...
  notes: {
    ...
    check: isString,
    display: ["song", "print"],
    // placeholder: when this field is absent, buildSongFile emits
    // `notes: <first-line>\n<rest>` instead of the default `# notes: TK`.
    // The block scalar format (`>-`) lets the user start typing multi-line
    // notes without restructuring the YAML.
    placeholder: ">-\n  TK",
  },

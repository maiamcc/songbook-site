...
// { desc?, values: { <key>: <description> } } — `desc` is optional
// (omit it for an enum whose field-level help isn't written yet);
// `values` must be a map of strings. Each per-value description is
// required if its key appears, so the legend never silently drops a
// row. Anything else throws at module load so a malformed YAML
// fails the build with a clear pointer instead of producing an
// empty-tooltip render downstream.
function loadEnums(yamlStr) {
  ...
  for (const [field, body] of Object.entries(raw || {})) {
    ...
    out[field] = { desc: body.desc ?? null, values: body.values };
  }
  return out;
}

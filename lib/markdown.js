...
export function configureMarkdown(md) {
    ...
    for (let i = 0; i < state.tokens.length; i++) {
      ...
      for (let j = 0; j < runs.length; j++) {
        ...
        if (run.type === "verse") {
          ...
          // Render each line separately and join with \n rather than calling
          // renderInline on the joined string. With breaks:true, renderInline
          // emits <br>\n between lines, and the \n is a second line break
          // under white-space:pre-wrap on .chorus.
          const htmlContent = run.lines
            .map((l) => md.renderInline(l.replace(/^(\t| {4})/, "")))
            .join("\n");
          const html = new state.Token("html_block", "", 0);
          html.content = `<div class="${cls}">${htmlContent}</div>\n`;
  ...
  // A code_block token immediately preceding or following a verse paragraph
  // is a refrain, not a standalone chorus. Convert it to an html_block with
  // the appropriate refrain classes, mirroring what refrain_lines does for
  // mixed paragraphs. (code_block tokens are produced when the FIRST line of
  // a stanza is tab-indented; refrain_lines only handles inline tokens inside
  // paragraphs, so it never sees these.)
  //
  // Must run before refrain_lines so the original paragraph map info is still
  // intact — refrain_lines creates new tokens without maps, which would make
  // the adjacency check unreliable. paragraph_close always has map:null, so
  // we check tokens[i-2] (the inline token) for the prevIsVerse case.
  md.core.ruler.before("refrain_lines", "code_block_refrain", (state) => {
    const tokens = state.tokens;
    for (let i = 0; i < tokens.length; i++) {
      if (tokens[i].type !== "code_block") continue;
      const cbMap = tokens[i].map;
      if (!cbMap) continue;

      const nextTok = i < tokens.length - 1 ? tokens[i + 1] : null;
      const prevInline = i >= 2 && tokens[i - 1].type === "paragraph_close"
        ? tokens[i - 2] : null;

      const nextIsVerse = nextTok?.type === "paragraph_open" &&
        nextTok.map && nextTok.map[0] === cbMap[1];
      const prevIsVerse = prevInline?.type === "inline" &&
        prevInline.map && prevInline.map[1] === cbMap[0];

      if (!prevIsVerse && !nextIsVerse) continue;

      const cls = nextIsVerse ? "chorus refrain refrain-mid" : "chorus refrain";
      const lines = tokens[i].content.replace(/\n+$/, "").split("\n")
        .map((l) => md.renderInline(l)).join("\n");
      const html = new state.Token("html_block", "", 0);
      html.content = `<div class="${cls}">${lines}</div>\n`;
      tokens[i] = html;

      if (prevIsVerse) {
        for (let j = i - 2; j >= 0; j--) {
          if (tokens[j].type === "paragraph_open") {
            if (!tokens[j].attrGet("class")) tokens[j].attrSet("class", "verse-pre-refrain");
            break;
          }
        }
      }
  ...
  md.renderer.rules.code_block = (tokens, idx) =>
    tokens[idx].content
      .replace(/\n+$/, "")
      .split(/\n\n+/)
      .map((part) => `<div class="chorus">${part.split("\n").map((l) => md.renderInline(l)).join("\n")}</div>`)
      .join("\n") + "\n";
